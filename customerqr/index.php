                         ...
                         foreach ($resultado as $rowheaders) 
                         {
                            $vprojectname = $rowheaders[0];
					          $phpValue = json_decode($rowheaders[0]);
                                ...
                                //echo "HOla". $phpValue->resultado;
                              
                              if ($phpValue->resultado==1)
                              {
                                 header("Location: https://www.flexbda.com/customersqr/home.php?sn=".$sanitized_sn."&mkey=".$sanitized_mkey);
                            //    header("Location: https://webfas.honeywell.com/customersqr/home.php?sn=".$sanitized_sn."&mkey=".$sanitized_mkey);
                            }
...
/*
        ...
        echo json_encode($reqmm);
            ...
            $ch = curl_init($v_url_get_run_id_save_outcome );        
            curl_setopt($ch, CURLOPT_POST, true);
            curl_setopt($ch, CURLOPT_POSTFIELDS, $jsonoutcome_data);
            ...
            if (curl_errno($ch)) {
              ...
              print_r($responseData); // Process or display the API response
            }
