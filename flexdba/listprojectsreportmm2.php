   ...
   //   echo "aaaaaaaaaaaaaaaaaaaaaaaaaaa"
      ...
      $vinstallationduedate = substr ($rowheaders['installationduedate'],0,10);
                    ...
                    <div class="">
                         ...
                         <h5>ESD: <?php echo $nameuserfull;  ?> &nbsp;</h5>[<?php echo  $nameuserfullusermail;?>]
                         <p class="text-muted">
                              ...
                              <tbody>
                                <tr>
                                  ...
                                  <td><?php 
                                   ...
                                   if($valarm_brand =="farenhyte") { echo "Farenhyte"; }
...
{
?>                      
<table class="table table-striped table-sm d-none " id="sss">
    ...
    <table class="table table-striped table-sm d-none " id="tablebugdet">    
    <tr>
  ...
  $sql = $connect->prepare(  $sqlbuscabda);
    ...
    foreach ($resultadomm as $row2) {
      $entro=1;
         ...
         $detect_description =  $row2['description'];
         $detect_price =  $row2['price'];
    break;
     }
   //  echo "-------------a1";
     if (  $entro ==1)
...
//echo $sqlbuscabda."<br>";
  ...
  <!--DAS ANTENA -->
  <?php
  $sqldasantena="select distinct   bandgroups.prioritybudget, flexbda_products_budget.*
	FROM public.flexbda_products_budget
	INNER JOIN bandgroups
	ON bandgroups.idband = flexbda_products_budget.idband
	inner join
	(
		select idbandgroup, count( distinct idband) as cc from bandgroups group by idbandgroup
	) as cantxgrupoband
	on cantxgrupoband.idbandgroup = bandgroups.idbandgroup  
	
	where  (description  LIKE '%Antenna%' OR description  LIKE '%Indoor%'  )
	and idgroupband in (SELECT bandgroups.idbandgroup  from flexbdahoneywell_band 
						INNER JOIN bandgroups
						ON bandgroups.idband = flexbdahoneywell_band.idband
						inner join (
							SELECT flexbdahoneywell_band.idproject, count(distinct flexbdahoneywell_band.idband) as ccp  from flexbdahoneywell_band 

											WHERE idproject = 1 AND idrev = 26 AND typeref = 'FREQBAND' 
											group by idproject
						) as cantbandxproject
						on cantbandxproject.idproject = 1 and
						cantxgrupoband.cc = cantbandxproject.ccp and
						bandgroups.grouped >= cantbandxproject.ccp 
						
						WHERE flexbdahoneywell_band.idproject = 1 AND idrev = 26 AND typeref = 'FREQBAND' AND typegroup ='MODULE')
		order by bandgroups.prioritybudget desc , price desc 
  ";
 //echo $sqldasantena;
 $elmontoamostrar = 0;
 $elmontoamostrarsolito =  0;
...
// echo "aaa111";
 ...
 <?php
  ...
  $sqldasantena="select distinct   bandgroups.prioritybudget, flexbda_products_budget.*
	...
	where  (description  LIKE '%COUPLER%' and  description  LIKE '%6dB%'  )
	and idgroupband in (SELECT bandgroups.idbandgroup  from flexbdahoneywell_band 
						INNER JOIN bandgroups
						ON bandgroups.idband = flexbdahoneywell_band.idband
						inner join (
...
<?php
     ...
     $detect_partnumberhoneywel="BDA-ICA12-JPLLR-1K";
     $detect_description='1000 Plenum Red Cable 1/2" Aluminum,RFS  ICA12-50JPLLR';
     $quantity = ceil($v_resul_dascoaxcable/1000);
     $elmontoamostrar =  ( 40.95 * $quantity );
     $elmontoamostrarsolito = "40.95"; 
     $canthrlow =0;
     $canthrhig = 0;
     $sentenciaaction = $connect->prepare("INSERT INTO public.flexbdahoneywell_bugdet(idproject, idrev, sku, description, quantity, netprice, nettotal, esdlaborhow, esdlaborhigh, orderreport)  VALUES (:idproject, :idrev, :sku, :description, :quantity, :netprice, :nettotal, :esdlaborhow, :esdlaborhigh,:orderreport);");
     $sentenciaaction->bindParam(':idproject', $vidp);
...
?>
    <tr>
      <th scope="row">BDA-ICA12-JPLLR-1K</th>
      <td scope="row">1000' Plenum Red Cable 1/2" Aluminum,RFS  ICA12-50JPLLR</td>
      <td><?php 
      echo $quantity; ?></td>
      <td><?php $price_cable = 40.95; echo  $price_cable; ?></td>
...
$detect_partnumberhoneywel="BDA-GKCK-10";
$detect_description='Coaxial Cable Grounding Kit';
$price_cable = 1039.5;
...
?>
    <tr>
      <th scope="row">BDA-GKCK-10</th>
      <td scope="row">Coaxial Cable Grounding Kit</td>
      <td><?php 
      
      echo $v_resul_donorantenncount; ?></td>
      <td><?php $price_cable = 1039.5; echo  $price_cable; ?></td>
...
*/
$detect_partnumberhoneywel="BDA-WPK-ATBC40_01";
$detect_description='Weatherproofing Silicone Coldshrink for EOL Assembly, 9.8"L ';
$price_cable = 1;
$quantity =$v_resul_donorantenncount;
$elmontoamostrar =  ($price_cable * $quantity );
$elmontoamostrarsolito = "1"; 
$canthrlow =($v_resul_dasantennacount* 1);
$canthrhig =($v_resul_dasantennacount* 1);
$sentenciaaction = $connect->prepare("INSERT INTO public.flexbdahoneywell_bugdet(idproject, idrev, sku, description, quantity, netprice, nettotal, esdlaborhow, esdlaborhigh, orderreport)  VALUES (:idproject, :idrev, :sku, :description, :quantity, :netprice, :nettotal, :esdlaborhow, :esdlaborhigh,:orderreport);");
$sentenciaaction->bindParam(':idproject', $vidp);
...
?>
    <tr>
      <th scope="row">BDA-WPK-ATBC40_01</th>
      <td scope="row">Weatherproofing Silicone Coldshrink for EOL Assembly, 9.8"L </td>
      <td><?php 
 ...
 <?php
  ...
  $sqldasantena="select distinct    flexbda_products_budget.*
	...
	where  (description  LIKE '%surge%' and  description  LIKE '%Coaxial%' and  description  LIKE '%protector%' and  description  LIKE '%UL%'  )
  ";
 ...
 <?php
  ...
  $sqldasantena="select distinct    flexbda_products_budget.*
	...
	where  (description  LIKE '%Jumper%' and  description  LIKE '%Coaxial%' and  description  LIKE '%18%' and  description  LIKE '%UL%'  )
  ";
...
echo "error".$v_resul_jumpercablecount; 
  ...
  $sqldasantena="select distinct   bandgroups.prioritybudget, flexbda_products_budget.*
	...
	where  (description  LIKE '%Donor%' and description  LIKE '%Antenna%'  )
	and idgroupband in (SELECT bandgroups.idbandgroup  from flexbdahoneywell_band 
						INNER JOIN bandgroups
						ON bandgroups.idband = flexbdahoneywell_band.idband
						inner join (
...
<?php  if ($v_result_bdasuggestion =="MM_UHFVHF")
{
?>
...
foreach ($resultado as $rowheaders) 
            {
              ...
              ?>
                <tr class='tablitamam<?php echo $orderreport; ?>'>
                  ...
                  <td>
                  <button class="btn btn-outline-primary btn-xs" data-toggle="modal" data-target="#myModal" contenteditable="false">Edit</button>
...
<?php 
echo "aca teneomos".$vactive;
echo "aaaaaaaaaa".$activeproy;
                         if ( $estaaprobado == "NAP" && $vactive <> "Y" )
                         {
 ...
 // console.log(result);
  ...
  /*  Swal.fire({
	...
	Swal.fire({
          ...
          /// Refrescamoss
          window.location = 'https://flexbda.com/listprojectsreportmm.php?idtoke='+vvidp+'&idr='+vvipr+'&st=NAP';
...
$.ajax
          ...
          if (data.resultiu = 'ok')
          {
            ...
            window.location = 'https://flexbda.com/listprojectsreportmm.php?idtoke='+vidpp+'&idr='+vidpprev+'&st=NAP';
